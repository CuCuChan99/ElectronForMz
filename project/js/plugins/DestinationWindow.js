...
// ----------------------------------------------------------------------------
// Version
// 2.3.0 2026/07/17 文字数に合わせてウィンドウの横幅を自動で伸縮させる「横幅自動調整」機能を追加（改変版）
// 2.2.2 2025/09/07 表示フレーム数のプラグインパラメータの型指定がiconになっていた問題を修正
 ...
 * @param autoFitWidth
 * @text 横幅自動調整
 * @desc 表示する文字列の長さに合わせてウィンドウの横幅を自動で伸縮させます。文字列揃えの設定が伸縮時の基準位置になります。
 * @default false
 * @type boolean
 *
 * @param showingFrames
 ...
 * \i[n]、\c[n]、\{、\}
 *
 * 横幅自動調整を有効にした場合、文字列の長さに合わせてウィンドウの横幅が
 * 自動で伸縮します。パラメータ「ウィンドウ横幅」で指定した領域を基準に、
 * 文字列揃えの設定に応じて以下の位置を基準に伸縮します。
 * ・左揃え　：ウィンドウの左端を固定
 * ・中央揃え：ウィンドウの中央を固定
 * ・右揃え　：ウィンドウの右端を固定
 * ※横幅自動調整と自動調整の両方を有効にした場合、横幅自動調整が優先され
 * 　文字は縮小されません。メニュー画面のウィンドウには適用されません。
 *
...
(()=> {
    ...
    Window_Destination.prototype.updateText = function() {
        ...
        const changed = textList.length !== this._textList.length ||
            iconIndex !== this._iconIndex ||
            textList.some((text, index) => this._textList[index] !== text);
        if (!changed) {
            return;
        }
        this._textList  = textList.clone();
        this._iconIndex = iconIndex;
        if (this.isFitWidth()) {
            this.updateFitWidth(textList, iconIndex);
        }
    ...
    Window_Destination.prototype.isFitWidth = function() {
        return !!param.autoFitWidth;
    };

    Window_Destination.prototype.updateFitWidth = function(textList, iconIndex) {
        const textWidth = this.findMaxTextWidth(textList, iconIndex);
        if (textWidth <= 0) {
            return;
        }
        const newWidth = Math.min(textWidth + this.padding * 2 + 8, Graphics.width);
        if (param.textAlign === 1) {
            this.x = param.windowX + Math.floor((param.windowWidth - newWidth) / 2);
        } else if (param.textAlign === 2) {
            this.x = param.windowX + param.windowWidth - newWidth;
        } else {
            this.x = param.windowX;
        }
        this.width = newWidth;
    };

    Window_Destination.prototype.findMaxTextWidth = function(textList, iconIndex) {
        return textList.reduce((maxWidth, text, index) => {
            let width = this.textSizeEx(text).width;
            if (iconIndex > 0 && index === 0) {
    ...
    Window_Destination.prototype.drawDestination = function(lineNumber) {
        ...
        if (param.autoAdjust && !this.isFitWidth()) {
            this.resetTextColor();
    ...
    Window_Destination.prototype.getContentsX = function() {
        ...
        const useAutoAdjust = param.autoAdjust && !this.isFitWidth();
        let width = useAutoAdjust ? this.textWidth(this._text) : this.textSizeEx(this._text).width;
    ...
    Window_DestinationMenu.prototype.isFitWidth = function() {
        return false;
    };
